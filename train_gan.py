...
# -------------------------------------------
# I've included the model definitions here for a complete, single-file script.
# This is the same architecture from our previous discussion.
...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels, down=True, act="relu", use_dropout=False):
        super().__init__()
        if down:
            # Downsampling block remains the same
            self.conv = nn.Sequential(
            ...
            # Upsampling block is now replaced with this better method
            self.conv = nn.Sequential(
